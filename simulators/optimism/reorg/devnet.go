...
func (d *Devnet) Start() {
	...
	var eth1, l2, opseq, opverif, l2os, bss *hivesim.ClientDefinition
	for _, client := range clientTypes {
		...
		if client.HasRole("op-sequencer") {
			opseq = client
		}
		if client.HasRole("op-verifier") {
			opverif = client
		}
	...
	if eth1 == nil || l2 == nil || opseq == nil || opverif == nil || l2os == nil || bss == nil {
		d.t.Fatal("op-l1, op-l2, op-proposer, op-batcher, op-sequencer, op-verifier required")
	}
	...
	d.nodes["op-l1"] = eth1
	d.nodes["op-l2"] = l2
	d.nodes["op-sequencer"] = opseq
	d.nodes["op-verifier"] = opverif
...
func (d *Devnet) InitOpSequencer() error {
...
func (d *Devnet) StartOpSequencer() error {
	op := d.nodes["op-sequencer"]
	...
	d.opseq = &OpSequencer{d.t.StartClient(op.Name, opts...)}
	return nil
}

func (d *Devnet) StartOpVerifier() error {
	op := d.nodes["op-verifier"]
	...
	optimismPortalOpt := hivesim.WithDynamicFile("/OptimismPortalProxy.json", bytesSource([]byte(d.optimismPortal)))
	opts := []hivesim.StartOption{executionOpts, optimismPortalOpt}
	d.opverif = &OpVerifier{d.t.StartClient(op.Name, opts...)}
	return nil
}
